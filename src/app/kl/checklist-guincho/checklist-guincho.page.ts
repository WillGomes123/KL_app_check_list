...
@Component({
  ...
  templateUrl: './checklist-guincho.page.html',
  styleUrls: ['./checklist-guincho.page.scss'],
})
export class ChecklistGuinchoPage {
  ...
  async salvarChecklist() {
    ...
    if (this.checklist.km_check !== undefined && this.checklist.km_check !== null) {
      if (this.checklist.gasolina !== undefined && this.checklist.gasolina !== null) {
        if (this.image !== undefined || this.image2 !== undefined || this.image3 !== undefined) {
  ...
  enviarAssinaturaGuincho() {
    if (this.image3 != undefined || this.image3 != "" || this.image3 != null) {
      var data2 = {
        imagem: this.image3,
        nome_img: this.createFileNameAssinatura(),
        pasta_arquivo: "assinaturas",
        formato: "png",
        tipo_assinante: "funcionario",
        id_assinante: this.usuario,
        momento_assinatura: this.checklist.tipo_check,
        id_checklist: this.id_checklist,
      };
      this.apiKlService.enviarImagen(data2).subscribe((data2: any) => {
        if (data2.status == false) {
          this.toastService.showError("Aconteceu um erro ao salvar assinatura guincho");
        }
         else {
          alert("checkList Guincho Salvo com Sucesso");
  ...
  async assinaturaGuincho(id: number) {
    const modal = await this.modalController.create({ component: SignatureComponent, componentProps: { id: id } });
